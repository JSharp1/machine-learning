...
for epsilon = min(pval):stepsize:max(pval)
    ...
    % F1 score is computed using precision (prec) and recall (rec):
    ...
    predictions = (pval < epsilon);%our predicted output
    
    tp = sum((predictions == 1) & (yval == 1));% predicted as anomaly, ground truth is anomaly.
    fp = sum((predictions == 1) & (yval == 0));% predicted as anomaly, ground truth is not anomaly
    fn = sum((predictions == 0) & (yval == 1));% predicted as anomaly, ground truth is not anomaly
